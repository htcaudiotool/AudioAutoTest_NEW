...
def trigger_asr(serialno):
    out, _ = subprocess.Popen("adb -s {} shell ps -A | grep 'audioserver'".format(serialno), shell = True, stdout = subprocess.PIPE).communicate()
    i1 = out.find("audioserver  ")
    i2 = out.find("     ")
    pid_number = out[i1 + len("audioserver  "):i2]
    
    if pid_number is not None:
        os.system("adb -s {} shell kill {} > /dev/null".format(serialno, pid_number))
        log("asr success")
...
def run(num_iter=1):
    ...
    while num_iter > 0:
        ...
        trials_batch = []
        trials_batch += playback_task_run(device, serialno, num_iter=min([num_iter, BATCH_SIZE]))
